...
class KISWebSocket:
    ...
    def __init__(self, price_codes=None, orderbook_codes=None):
    ...
    async def do_pingpong(self, ws):
        """PINGPONG 메시지 처리 - 서버에서 PINGPONG이 오면 그대로 돌려보냄"""
        await ws.send("PINGPONG")  # 서버에게 PONG 응답
            ...
            while True:
                try:
                    ...
                    if data:
                        tr_id = data.get("header", {}).get("tr_id")
                        if tr_id == "PINGPONG":
                            await self.do_pingpong(ws)
                            continue
                        else :
                            self.setting_crypto_info(data)
                            continue
